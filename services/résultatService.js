...
async function FindSingleRésultat(résultatId, résultatData) {
    try {
      const résultat = await Résultat.findById(résultatId, résultatData, { new: true });
      if (!résultat) {
...
async function UpdateRésultat(résultatId, résultatData) {
    try {
      const résultat = await Résultat.findByIdAndUpdate(résultatId, résultatData, { new: true });
      if (!résultat) {
        throw new Error('résultat not found');
      }
      return résultat;
